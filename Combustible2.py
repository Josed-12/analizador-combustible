...
st.markdown("Please upload your telemetry CSV or ZIP file to process the pump data")
st.markdown("---")

# --- BARRA LATERAL (Controles) ---
st.sidebar.header("Visualization settings")
# Se actualizó para aceptar .zip
uploaded_file = st.sidebar.file_uploader("1. Upload your file", type=["csv", "zip"])
...
if mode in ["Consumption per pump", "HP Consumption/Performance"] and uploaded_file is not None:
    ...
    def get_pump_names(file):
        # Manejo de ZIP para lectura de encabezados
...
def process_data(file, factor):
    # Lógica de extracción si es ZIP
    if file.name.endswith('.zip'):
        with zipfile.ZipFile(file) as z:
            csv_files = [f for f in z.namelist() if f.endswith('.csv')]
            if not csv_files:
    ...
    for pid in pump_ids:
        r_cols = [c for c in df_raw.columns if pid in c and 'consumption_rate' in c and '|UOM' not in c]
        if r_cols:
            data_map['rate'][pid] = pd.to_numeric(df_raw[r_cols[0]], errors='coerce').fillna(0) * factor
...
if uploaded_file is not None and plot_button:
    processed_output = process_data(uploaded_file, conversion_factor)

    if processed_output:
        df, data_map, time_diff, df_plot, step = processed_output
        ...
        if mode in ["Consumption per pump", "HP Consumption/Performance"]:
            for i, pid in enumerate(pumps_to_show):
                ...
                if mode == "HP Consumption/Performance":
                    if pid in data_map['power']:
                        fig.add_trace(go.Scatter(x=df_plot['timestamp'], y=data_map['power'][pid].iloc[::step],
                                                 name=f"{pid} Power", line=dict(width=1.5, color=c), showlegend=False),
                                      row=3, col=1)
                    if pid in data_map['load']:
                        fig.add_trace(go.Scatter(x=df_plot['timestamp'], y=data_map['load'][pid].iloc[::step],
                                                 name=f"{pid} Load", line=dict(width=1.5, color=c), showlegend=False),
                                      row=4, col=1)
        ...
        if mode == "Per minute":
            resampled = pd.DataFrame({'r': current_total_rate, 't': df['timestamp']}).set_index('t').resample(
                '1min').mean()
    ...
    st.info("👈 Please upload a telemetry CSV or ZIP file to start.")